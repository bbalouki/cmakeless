...
5. **Teach the emitter a trick.** Pick a CMake construct we generate suboptimally and make the output more idiomatic. The Visitor structure ([ARCHITECTURE.md](ARCHITECTURE.md#design-patterns-named)) keeps these changes local.
...
7. **Take a roadmap item.** Phase items in [ROADMAP.md](ROADMAP.md) are deliberately scoped to be ownable by one motivated person.
