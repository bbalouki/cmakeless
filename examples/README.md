...
| [11_portability](11_portability/) | The portability release: a curated `Toolchain` gallery (`Toolchain.arm_none_eabi()`, `Toolchain.ios()`, and, when their SDKs are present, `Toolchain.emscripten()`/`Toolchain.android(ndk=...)`) each registered alongside a preset that references it, and `project.lint(clang_tidy=True)` wired in when clang-tidy is on PATH. The default `python cmakelessfile.py`/`cmakeless build` only ever builds for the host; try `cmakeless doctor` (checks cmake/generator/ccache/vcpkg/Conan/network, no project needed), `cmakeless lock && cmakeless sbom --format spdx` (a CycloneDX/SPDX bill of materials from `cmakeless.lock`), and `cmakeless vendor` followed by `cmakeless build --offline` (zero-network build from the vendored copy). |
| [12_cmake_globals](12_cmake_globals/) | Query any CMake variable: `project.cmake_globals()` runs a throwaway CMake configure immediately and hands back every variable it defined, so `if hasattr(cmake, "WIN32"): ...` (mirroring CMake's `if(DEFINED WIN32)`, not `if(WIN32)`) can steer `app.define(...)` before a single line of CMake is emitted. |
| [13_cxx_modules](13_cxx_modules/) | C++20 modules: a library whose entire interface is module interface units, declared with `modules=[...]` and emitted as a `target_sources(... FILE_SET CXX_MODULES ...)` block, imported by name from the executable with no header and no include directory anywhere. Because the project declares modules, its generated `CMakeLists.txt` asks for CMake 3.28 instead of the usual 3.25; projects that declare none are unaffected. `cmakeless doctor` reports whether the local CMake and generator can build it. |
